...
function App() {
  ...
  const logTraining = async () => {
    ...
    else if (nextSessions >= 30) nextRank = "Gold Prajiat"; // Updated rank name
    else if (nextSessions >= 10) nextRank = "Blue Prajiat";

    await updateDoc(userRef, { sessions: increment(1), rank: nextRank });
  };

  if (loading) return <div className="h-screen bg-black flex items-center justify-center text-yellow-500 font-black tracking-[.5em] animate-pulse italic">INITIALIZING DOJO...</div>;

  return (
    <div className="min-h-screen bg-zinc-950 text-white selection:bg-yellow-600 overflow-x-hidden">
        ...
        {!user ? (
          ...
          >
            ...
            <div className="relative z-20 w-full max-w-[400px] flex flex-col items-center">
              {/* Gold Title */}
              <h1 className="text-6xl font-black italic tracking-tighter uppercase mb-10 text-center drop-shadow-[0_0_15px_rgba(234,179,8,0.7)]">
              ...
              <button 
                ...
                className="mt-8 text-[10px] text-zinc-400 uppercase tracking-widest font-bold italic underline hover:text-yellow-500 transition-colors"
              >
          ...
          <motion.div key="system" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col min-h-screen">
            <nav className="border-b border-zinc-800 bg-black/80 backdrop-blur-xl p-4 flex justify-between items-center sticky top-0 z-50">
              <div className="flex items-center gap-4">
                ...
                <div className={`px-2 py-0.5 text-[9px] font-black uppercase tracking-widest ${isAdmin ? 'bg-yellow-500 text-black shadow-[0_0_10px_rgba(234,179,8,0.3)]' : 'bg-zinc-800 text-zinc-400'}`}>
                  {isAdmin ? "MASTER / KRU" : "Fighter"}
                </div>
              </div>
              <button onClick={() => signOut(auth)} className="bg-zinc-900 p-2 hover:bg-yellow-500 hover:text-black transition-all border border-zinc-800"><LogOut size={18} /></button>
            </nav>

            <main className="flex-1 p-6 lg:p-10 max-w-7xl mx-auto w-full space-y-10">
              {isAdmin && (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="bg-yellow-500 text-black p-6 shadow-2xl relative group overflow-hidden">
                  ...
                  <div className="bg-zinc-900 border border-zinc-800 p-6 group cursor-pointer hover:border-yellow-500 transition-all">
                ...
                <div className="lg:col-span-4 space-y-6">
                  <div className="bg-zinc-900 border border-zinc-800 p-8 shadow-2xl">
                    ...
                    <div className="mt-8 space-y-6">
                      {/* Red line (border-l-4 border-red-600) removed below */}
                      <div className="bg-black p-5 border border-zinc-800 shadow-xl relative overflow-hidden group">
                ...
                <div className="lg:col-span-8">
                  <div className="bg-zinc-900 border border-zinc-800 p-8 h-full shadow-2xl">
                    <div className="flex justify-between items-center mb-8 border-b border-zinc-800 pb-4">
                      <h4 className="text-2xl font-black italic uppercase flex items-center gap-2 tracking-tighter"><Clock size={22} className="text-yellow-500" /> Daily Intel</h4>
                      {isAdmin && (
                        <button onClick={() => setIsEditingSchedule(!isEditingSchedule)} className="text-xs font-black text-zinc-500 hover:text-white flex items-center gap-2 uppercase tracking-widest underline decoration-yellow-500">
                    ...
                    <div className="space-y-4">
                      {(gymSettings.schedule && gymSettings.schedule.length > 0 ? gymSettings.schedule : ['No Intel Available']).map((item, i) => (
                        <div key={i} className="flex justify-between items-center p-5 bg-black border border-zinc-800 group hover:bg-zinc-900 transition-colors">
                          ...
                          <span className="font-mono text-yellow-500 font-bold bg-yellow-500/10 px-2 py-1 italic border border-yellow-500/20">LIVE DATA</span>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                <div className="bg-zinc-900 border border-zinc-800 overflow-hidden shadow-2xl">
                  ...
                  <table className="w-full text-left text-xs uppercase font-bold tracking-tight">
                    <tbody className="divide-y divide-zinc-800">
                      {fighters.map(f => (
                        <tr key={f.id} className="hover:bg-yellow-500/5 transition-colors group">
                          <td className="p-4 italic font-black group-hover:text-yellow-500">{f.email}</td>
                          <td className="p-4 text-zinc-500">{f.rank || 'White Prajiat'}</td>
                          <td className="p-4 text-right text-yellow-500 hover:text-white cursor-pointer italic font-black">Manage_System</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
